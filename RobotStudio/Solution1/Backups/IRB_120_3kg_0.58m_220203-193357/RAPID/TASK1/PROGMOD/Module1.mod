MODULE Module1
    ...
    PROC main()
        ...
        WHILE ok=TRUE DO
            SocketSend my_socket,\Str:="Hola holita vecinito"; !escribe en la red y lo envia a arduino (donde será leido)
            WaitTime 0.5; !espera un tiempo
            SocketReceive my_socket \RawData := receive_string,\Time:=WAIT_MAX;

            UnpackRawBytes receive_string, 1, string1 \ASCII:=15; !7 es el espacio en caracteres que forman el número
